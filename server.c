...
int main () 
{
    ...
    if (bind(sd, (struct sockaddr *)&sock_path, sizeof(sock_path.sun_path)) == -1)
    {
        perror("Socket bind\n");
        exit(1);
    }
    ...
    *socklen = sizeof(sock_path.sun_path);
    if (accepted_socket = accept(sd, (struct sockaddr *)&sock_path, socklen) == 0)
    {
    ...
    //if (read(accepted_socket, recieved_msg, 1) == -1)
    //{
    //    perror("Socket read\n");
    //    exit(1);
    //}
    read(accepted_socket, recieved_msg, 1);
    printf("MESSAGE\n");
